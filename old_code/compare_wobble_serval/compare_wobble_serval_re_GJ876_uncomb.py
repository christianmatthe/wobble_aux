...
kt='Kt3_orders[11,32)'
pp =PdfPages("wobble_servalcomparison_GJ876_def_"+kt+".pdf")
...
for i in objects:
    ...
    if i[2]=="vis":
        ...
        N_ord=10 # note CARMENES has 61 orderes only 11-52 used here
        k=np.arange(N_ord)
        keys_orderRVs=[]
        for j in k:
    ...
    if True:
        for n in tqdm(range(len(w_RVs_barycorr_ivar))):
            w_RVs_barycorr_ivar[n]=bary.get_BC_vel(w_dates[n], starname=i[0], lat=_lat, longi=_lon, alt=_elevation, zmeas=w_RVs_own[n]/lightvel)[0]
    ...
    w_RVs_original = np.zeros(len(indices_wobble))
...
# =============================================================================
    ...
    ax1.plot(ser_rvc[:,0], (ser_rvc[:,5] -np.nanmean(ser_rvc[:,5])) - (w_RVs_barycorr_ivar-np.nanmean(w_RVs_barycorr_ivar)), "x", label= "w_ivar_weighted")
    ax1.plot(ser_rvc[:,0], (ser_rvc[:,5] -np.nanmean(ser_rvc[:,5])) - (w_RVs_barycorr -np.nanmean(w_RVs_barycorr)), "x",label= "w_median")
