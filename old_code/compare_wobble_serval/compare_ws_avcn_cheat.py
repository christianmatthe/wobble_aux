...
#orbital_parameters = [37.02, 41.382, 0.380, 253.6, 2450000 + 1581.046]# from carmenes data itself
#objects = [["GJ1148", "J11417+427", "vis", 41.382]]

#objects = [["GJ876", "J22532-142", "vis", 61.03]]

objects = [["GJ436", "J11421+267", "vis", 2.643859]]
#orbital_parameters = [17.38, 2.644, 0.152, 325.8, 2450000 + 1552.077]
...
#kt='Kt3_opt3'
pp =PdfPages("wobble_servalcomparison_"+"avcn"+objects[0][0]+ kt +".pdf")
...
for i in objects:
    ...
    if load_rvs:
        try:
            f = open("rv_files/"+i[0] + "_" + i[2] +kt+ "_bervs_unweight.pickle", "rb")
            w_RVs_barycorr = pickle.load(f)
            f.close()
            
            f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_ivar.pickle", "rb")
            w_RVs_barycorr_ivar = pickle.load(f)
            f.close()
            
            f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_orig.pickle", "rb")
            w_RVs_original_barycorr = pickle.load(f)
            f.close()
            
            f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_orders.pickle", "rb")
    ...
    #export to pickle
        ...
        f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_unweight.pickle", "wb")
        pickle.dump(w_RVs_barycorr, f)
        f.close()
        
        f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_ivar.pickle", "wb")
        pickle.dump(w_RVs_barycorr_ivar, f)
        f.close()
        
        f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_orig.pickle", "wb")
        pickle.dump(w_RVs_original_barycorr, f)
        f.close()
        
        f = open("rv_files/"+i[0]+"_"+i[2]+kt+"_bervs_orders.pickle", "wb")
    ...
    ser_corr = - ser_rvc[:,8] - ser_rvc[:,3] - ser_rvc[:,9]
...
# =============================================================================
    ...
    if len(i) >= 4:
        ...
        if 'orbital_parameters' in locals():
            ...
            popt_s, pcov_s = sp.optimize.curve_fit(fit_func, xdata, ydata, sigma = ser_rvc[:,2], absolute_sigma = True,#p0=[-0.1348]
            p0=[0]
                                                   )
        ...
        ax1.errorbar((w_dates) % i[3], (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)), yerr = w_RVs_er,fmt = "x", label="Wobble_Corr, clipped_sigma = {0:.3f}".format(sigma_wob))
        ax1.plot((ser_rvc[:,0]) % i[3], ser_rvc[:,5] - np.nanmean(ser_rvc[:,5]), "x", label= "SERVAL")
        ax1.errorbar((ser_rvc[:,0]) % i[3], ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]),yerr = ser_rvc[:,2] ,fmt = "x", label= "SERVAL_Corr, clipped_sigma = {0:.3f}".format(sigma_ser))
    ...
    if 'orbital_parameters' in locals():
        ax1.errorbar((w_dates) % i[3], (w_RVs_original_barycorr - np.nanmean(w_RVs_original_barycorr) - fit_func(w_dates, T0_offset)), yerr = w_RVs_er,fmt = "x", label = "wobble_corr")
        ax1.errorbar((ser_rvc[:,0]) % i[3], ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - fit_func(ser_rvc[:,0], T0_offset_s),yerr = ser_rvc[:,2] ,fmt = "x",color = "C2", label = "SERVAL_corr")
        ...
        wob_residuals = np.absolute(w_RVs_original_barycorr - np.nanmean(w_RVs_original_barycorr) - fit_func(w_dates, T0_offset))
        wob_residuals_sign = w_RVs_original_barycorr - np.nanmean(w_RVs_original_barycorr) - fit_func(w_dates, T0_offset)
        ...
        ax1.plot(np.absolute(ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - fit_func(ser_rvc[:,0], T0_offset_s)), ser_rvc[:,2] , "x",color = "C2", label = "SERVAL_corr")
        ...
        ax1.plot(wob_residuals_sign, - ser_rvc[:,3],"x", label = "wobble_corr")
        ax1.set_ylabel("drift [m/s]")
        ax1.set_xlabel('RV residuals [m/s]')
        plt.title("RV residuals vs drift correlation for "+i[0]+" ("+i[2]+") "+" - "+i[1]+";")
        plt.grid(True)
        plt.tight_layout()
        plt.legend(shadow=True)
        plt.savefig(pp, format='pdf')
    ...
    sigma_difference = np.nanstd(sigma_clip(
        ...
        ,sigma = 5) )
    ax1.plot(ser_rvc[:,0], (ser_rvc[:,1] -np.nanmean(ser_rvc[:,1])) - (w_RVs_original_barycorr -np.nanmean(w_RVs_original_barycorr)) , "x", label="w_orig_bary, std of difference (clipped) = {0:.3f}".format(sigma_difference) )
