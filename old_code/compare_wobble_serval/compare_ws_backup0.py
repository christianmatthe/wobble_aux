...
# =============================================================================
    ...
    orders = w_orders + 5
    for ep in range(len(w_dates)):
        ax1.plot(np.zeros(N_ord) + w_dates[ep], ser_rvo[ep, orders], "x")
    
    ser_rv_scatter = np.nanstd(ser_rvc[:,5])
    ax1.plot(w_dates, ser_rvc[:,5], "ko", label = r'SERVAL $\sigma = $' + str(ser_rv_scatter) )
    #ax1.plot(w_dates, w_RVs_unweighted, "ko")
    ax1.set_ylabel('RVs per order and combined [m/s]')
    ax1.set_xlabel('jd')
    ...
    ax1.plot(ser_rvc[:,0], (ser_rvc[:,5] -np.nanmean(ser_rvc[:,5])) - (w_RVs_original_barycorr -np.nanmean(w_RVs_original_barycorr)) , "x", label="w_orig_bary")
