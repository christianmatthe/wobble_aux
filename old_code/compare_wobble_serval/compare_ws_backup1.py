...
kt='Kt3_n160_stitched_def'
pp =PdfPages("wobble_servalcomparison_"+objects[0][0]+'_'+ kt +".pdf")
...
for i in objects:
    ...
    w_RVs_barycorr_ivar = w_RVs_barycorr_ivar[indices_wobble]
    ...
    w_RVs_original_barycorr = w_RVs_original_barycorr[indices_wobble]
...
# =============================================================================
    ...
    for ep in range(len(w_dates)):
        ax1.plot(np.zeros(N_ord) + w_dates[ep], w_order_RVs[:,ep] + w_bervs[ep] - np.nanmean(w_RVs + w_bervs), "x")
    
    
    
    wob_rv_scatter = np.median(w_order_RV_scatter)
    ax1.plot(w_dates, w_RVs + w_bervs - np.nanmean(w_RVs + w_bervs), "ko",label = 'Wobble; scatter_median = ' + str(wob_rv_scatter))
    ...
    ax1.errorbar(
        ...
        w_RVs + w_bervs - np.nanmean(w_RVs + w_bervs) ,
    ...
    order = 31
    ind_rvo = order + 5
    ax1.plot(w_dates[:], (ser_rvo[:, ind_rvo] - np.nanmean(ser_rvo[:, ind_rvo])) - (w_order_RVs[order ,:] + w_bervs[:] -np.nanmean(w_order_RVs[order ,:] + w_bervs[:]) ), "x")
       
    #ax1.plot(w_dates, w_RVs + w_bervs, "ko")
    #ax1.plot(w_dates, w_RVs_unweighted, "ko")
    ax1.set_ylabel('RVs per order and combined [m/s]')
    ax1.set_xlabel('jd')
    plt.title("RV order " + str(order) +" w-s (quick Bervs)" +i[0]+" ("+i[2]+") "+" - "+i[1])
    plt.grid(True)
    plt.tight_layout()
    #plt.legend(shadow=True)
    plt.savefig(pp, format='pdf')
    ...
    sigma_difference = np.nanstd(sigma_clip(
        ...
        ,sigma = 5) )
    ax1.plot(ser_rvc[:,0], (ser_rvc[:,5] -np.nanmean(ser_rvc[:,5])) - (w_RVs_original_barycorr -np.nanmean(w_RVs_original_barycorr)) , "x", label="w_orig_bary, std of difference = {0:.3f}".format(sigma_difference) )
    ...
    for index, order in enumerate(w_orders[:-2]):
        ...
        (ser_rvo[:, ind_rvo] - np.nanmean(ser_rvo[:, ind_rvo])) - (w_order_RVs[order_ind ,:] + w_bervs[:] -np.nanmean(w_order_RVs[order_ind ,:] + w_bervs[:]))
         )
        sigma_difference_clipped = np.nanstd(sigma_clip(
        (ser_rvo[:, ind_rvo] - np.nanmean(ser_rvo[:, ind_rvo])) - (w_order_RVs[order_ind ,:] + w_bervs[:] -np.nanmean(w_order_RVs[order_ind ,:] + w_bervs[:]))
        ,sigma = 5) )
        ax1.plot(w_dates[:], (ser_rvo[:, ind_rvo] - np.nanmean(ser_rvo[:, ind_rvo])) - (w_order_RVs[order_ind ,:] + w_bervs[:] -np.nanmean(w_order_RVs[order_ind ,:] + w_bervs[:]) ), "x", label= r"std_of_difference(clipped) = {0:.3f}, $\kappa \sigma$ clipped = {1:.3f} ".format(sigma_difference, sigma_difference_clipped))
        
        #ax1.plot(w_dates, w_RVs + w_bervs, "ko")
        #ax1.plot(w_dates, w_RVs_unweighted, "ko")
        ax1.set_ylabel('RVs per order and combined [m/s]')
        ax1.set_xlabel('jd')
        plt.title("RV order " + str(order)+ "(i=" + str(order_ind) + ")" +" w-s (quick Bervs)" +i[0]+" ("+i[2]+") "+" - "+i[1])
        plt.grid(True)
        plt.tight_layout()
        plt.legend(shadow=True)
        plt.savefig(pp, format='pdf')
