...
order_plots = False
#TODO make just_pres_plots flag
...
kt_list = []
#these need drift correction on (TRUE)
#correct_w_for_drift = True
#kt = "Kt3_pres:GJ436_l4_reg"
#kt_list.append(kt)
#kt = "Kt3_pres:GJ436_l4_reg+snr"
#kt_list.append(kt)

#kt = "Kt3_pres:GJ1148_wob_reg"
#kt_list.append(kt)
#kt = "Kt3_pres:def_roll1"
#kt_list.append(kt)

###these need drift correction off (False)
#correct_w_for_drift = False
#kt = "Kt3_pres:GJ436_l4_reg+drift"
#kt_list.append(kt)
#kt = "Kt3_pres:GJ436_l4_reg+snr+drift"
#kt_list.append(kt)
...
for kt in kt_list:
    ...
    if use_avcn == True:
        ...
        avcn_tag = ""
    pp =PdfPages("pres_06112019/wobble_servalcomparison_" +objects[0][0] + avcn_tag + kt + extra_identifier+".pdf")
    ...
    for i in objects:
        ...
        if i[2]=="vis":
            ...
            fil = "/data/cmatthe/compare_wobble_serval/wobbledir/for_pres_06112019/results_"+i[0]+"_Kstar0_"+kt+".hdf5"
        ...
        ser_rvo = np.loadtxt(servaldir+i[1]+"/"+i[1]+".rvo.dat")# HACK match length if serval radar and spectra are not same ageand contain newer spectra [:165]
        ser_rvo = ser_rvo[ind_finitedrift]
        
        ser_rvo_err = np.loadtxt(servaldir+i[1]+"/"+i[1]+".rvo.daterr")# HACK [:165]
        ...
        w_order_RVs = w_order_RVs[:,indices_wobble]                             + ser_corr_wob
    ...
    # =============================================================================
        ...
        if len(i) >= 4:
            ...
            if 'orbital_parameters_mult' in locals():
                ...
                #BEGIN pres_plots multiplanet rework
                ax1.plot(xlst , fit_func_mult(xlst, popt_s[0]
                                                #,popt_s[1]
                                                ), "k-", label = "lit orbit (serval_fit)")
                ax1.errorbar(w_dates, (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)) - residual_offset , yerr = w_RVs_er,fmt = "x", label= r"Wobble_Corr, rms ={0:.3f}, $5\sigma$_clipped = {1:.3f}, 5err_clipped = {2:.3f}".format(wob_mult_stats[0], wob_mult_stats[1], wob_mult_stats[2]))
                #ax1.plot(ser_rvc[:,0], ser_rvc[:,5] - np.nanmean(ser_rvc[:,5]) - residual_offset, "x", label= "SERVAL")
                ax1.errorbar(ser_rvc[:,0], ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - residual_offset,yerr = ser_rvc[:,2] ,fmt = "x", label= r"SERVAL, rms ={0:.3f}, $5\sigma$_clipped = {1:.3f}, 5err_clipped = {2:.3f}".format(ser_mult_stats[0], ser_mult_stats[1], ser_mult_stats[2]))
                if error_clipping == True:
                    ind_bad_wob = wob_mult_stats[3]
                    ind_bad_ser = ser_mult_stats[3]
                    ax1.plot((w_dates[ind_bad_wob]), (w_RVs_original_barycorr[ind_bad_wob] - np.nanmean(w_RVs_original_barycorr) - residual_offset), "o",color = "C0", label = "error_clipped wobble points")
                    ax1.plot((ser_rvc[:,0][ind_bad_ser]), ser_rvc[:,1][ind_bad_ser] - np.nanmean(ser_rvc[:,1]), "o", color = "C1", label= "error_clipped serval points")
                #ax1.plot(w_dates % i[3], (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)),"x", label="Wobble_Corr")
                bottom, top = ax1.get_ylim()
                ax1.set_ylim([bottom - 0.2*(top-bottom), top]) #add additional plot range paddinng below to fit legend
                ...
                plt.tight_layout()
                plt.legend(shadow=True)
                plt.savefig("pres_06112019/"+"nonphased_multifit_plot_"+i[0] + avcn_tag + kt + extra_identifier+".pdf", format='pdf')#save just this plot
                plt.savefig(pp, format='pdf')
                plt.clf()
                fig.clf()
                ax1 = plt.gca()
                ...
                if primary_planet_phased_plot == True:
                    ...
                    # BEGIN Pres_Plot primary phased
                    ax1.plot(pltlst[0] % i[3], pltlst[1], "k-", label = "literature orbit (SERVAL fit)")
                    #ax1.plot(pltlst[0] % i[3], keplarian_rv(pltlst[0]), "k-", label = "lit orbit M0 (no fit) single planet")
                            
                    #ax1.plot(w_dates % i[3], (w_RVs_original+w_bervs-np.nanmean(w_RVs_original+w_bervs)),"x", label="Wobble_orig")
                    ax1.errorbar((w_dates) % i[3], (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)) - residual_offset, yerr = w_RVs_er,fmt = "x", label=r"Wobble, rms ={0:.3f}, $5\sigma$_clipped = {1:.3f}, 5err_clipped = {2:.3f} ".format(wob_stats[0], wob_stats[1], wob_stats[2]))
                    #ax1.plot((ser_rvc[:,0]) % i[3], ser_rvc[:,5] - np.nanmean(ser_rvc[:,5]), "x", label= "SERVAL")
                    ax1.errorbar((ser_rvc[:,0]) % i[3], ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - residual_offset,yerr = ser_rvc[:,2] ,fmt = "x", label= r"SERVAL, rms ={0:.3f}, $5\sigma$_clipped = {1:.3f}, 5err_clipped = {2:.3f}".format(ser_stats[0], ser_stats[1], ser_stats[2]))
                    if error_clipping == True:
                        ind_bad_wob = wob_stats[3]
                        ind_bad_ser = ser_stats[3]
                        ax1.plot((w_dates[ind_bad_wob]) % i[3], (w_RVs_original_barycorr[ind_bad_wob] - np.nanmean(w_RVs_original_barycorr))- residual_offset, "o",color = "C0", label = "error_clipped wobble points")
                        ax1.plot((ser_rvc[:,0][ind_bad_ser]) % i[3], ser_rvc[:,1][ind_bad_ser] - np.nanmean(ser_rvc[:,1]) - residual_offset, "o", color = "C1", label= "error_clipped serval points")
                    #ax1.plot(w_dates % i[3], (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)),"x", label="Wobble_Corr")
                    ax1.set_ylabel("RVs [m/s]")
                    ...
                    plt.tight_layout()
                    plt.legend(shadow=True)
                    plt.savefig("pres_06112019/"+"primary_planet_plot_"+i[0] + avcn_tag + kt + extra_identifier+".pdf", format='pdf')#save just this plot
                    plt.savefig(pp, format='pdf')
                    plt.clf()
                    fig.clf()
                    ax1 = plt.gca()
                    ...
                    # END Pres_Plot
                    # BEGIN GJ876 no fit plot
                    if i[0] == "GJ876":
                        #ax1.plot(pltlst[0] % i[3], pltlst[1], "k-", label = "literature orbit (SERVAL fit)")
                        #ax1.plot(pltlst[0] % i[3], keplarian_rv(pltlst[0]), "k-", label = "lit orbit M0 (no fit) single planet")
                                
                        #ax1.plot(w_dates % i[3], (w_RVs_original+w_bervs-np.nanmean(w_RVs_original+w_bervs)),"x", label="Wobble_orig")
                        wob_rms = np.std((w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)))
                        ser_rms = np.std(ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]))
                        
                        ax1.errorbar((w_dates) % i[3], (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)), yerr = w_RVs_er,fmt = "x", label=r"Wobble, rms ={0:.3f} ".format(wob_rms))
                        #ax1.plot((ser_rvc[:,0]) % i[3], ser_rvc[:,5] - np.nanmean(ser_rvc[:,5]), "x", label= "SERVAL")
                        ax1.errorbar((ser_rvc[:,0]) % i[3], ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]),yerr = ser_rvc[:,2] ,fmt = "x", label= r"SERVAL, rms ={0:.3f}".format(ser_rms))
                        if error_clipping == True:
                            ind_bad_wob = wob_stats[3]
                            ind_bad_ser = ser_stats[3]
                            ax1.plot((w_dates[ind_bad_wob]) % i[3], (w_RVs_original_barycorr[ind_bad_wob] - np.nanmean(w_RVs_original_barycorr)), "o",color = "C0", label = "error_clipped wobble points")
                            ax1.plot((ser_rvc[:,0][ind_bad_ser]) % i[3], ser_rvc[:,1][ind_bad_ser] - np.nanmean(ser_rvc[:,1]), "o", color = "C1", label= "error_clipped serval points")
                        #ax1.plot(w_dates % i[3], (w_RVs_original_barycorr-np.nanmean(w_RVs_original_barycorr)),"x", label="Wobble_Corr")
                        ax1.set_ylabel("RVs [m/s]")
                        ax1.set_xlabel('phase [d]')
                        plt.title("Phased ("+str(i[3])+"d) RVs for "+i[0]+" ("+i[2]+") "+" - "+i[1]+";")
                        ...
                        plt.tight_layout()
                        plt.legend(shadow=True)
                        plt.savefig("pres_06112019/"+"just_rms_"+i[0] + avcn_tag + kt + extra_identifier+".pdf", format='pdf')#save just this plot
                        plt.savefig(pp, format='pdf')
                        plt.clf()
                        fig.clf()
                        ax1 = plt.gca()
        ...
        if 'orbital_parameters' in locals():
            ...
            #BEGIN pres_plot_drift correction correlation
            ax1.plot(- ser_rvc[:,3],  wob_residuals_sign ,"x", label = "wobble_corr")
            if error_clipping == True:
                ax1.plot( -ser_rvc[:,3][ind_bad_wob],(wob_residuals_sign[ind_bad_wob]), "o",color = "C0", label = "error_clipped wobble points")
            ax1.plot([1.1*float(x) for x in range(int(min(wob_residuals_sign)),int(max(wob_residuals_sign)))],[1.1*float(x) for x in range(int(min(wob_residuals_sign)),int(max(wob_residuals_sign)))], "-", label = "slope 1", color="r")
            ax1.set_xlabel("Drift [m/s]")
            ax1.set_ylabel('RV residuals [m/s]')
            plt.title("RV residuals vs Drift Correction for "+i[0]+" ("+i[2]+") "+" - "+i[1]+";")
            plt.grid(True)
            plt.tight_layout()
            plt.legend(shadow=True)
            plt.savefig("pres_06112019/"+"Residuals_vs_Drift_"+i[0] + avcn_tag + kt + extra_identifier+".pdf", format='pdf')#save just this plot
            plt.savefig(pp, format='pdf')
            plt.clf()
            fig.clf()
            ax1 = plt.gca()
