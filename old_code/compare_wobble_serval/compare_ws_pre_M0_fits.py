...
#orbital_parameters = [212.07, 61.082, 0.027, 35.1, 2450000 + 602.093]

objects = [["GJ436", "J11421+267", "vis", 2.643859]]
orbital_parameters = [17.38, 2.644, 0.152, 325.8, 2450000 + 1552.077] #(Try with M0 offset of 78.3 deg (see Carm paper))
...
pp =PdfPages("wobble_servalcomparison_" +objects[0][0] + avcn_tag + kt +".pdf")
...
# =============================================================================
    ...
    if len(i) >= 4:
        ...
        if 'orbital_parameters' in locals():
            ...
            ser_residuals_sign = ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - fit_func(ser_rvc[:,0], T0_offset_s)
    ...
    if 'orbital_parameters' in locals():
        ...
        if error_clipping == True:
            ax1.plot((w_dates[ind_bad_wob]) % i[3], (w_RVs_original_barycorr - np.nanmean(w_RVs_original_barycorr) - fit_func(w_dates, T0_offset))[ind_bad_wob], "o",color = "C0", label = "error_clipped wobble points")
        ...
        ax1.errorbar((ser_rvc[:,0]) % i[3], (ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - fit_func(ser_rvc[:,0], T0_offset_s))/ser_rvc[:,2] ,yerr = ser_rvc[:,2]/ser_rvc[:,2]  ,fmt = "x",color = "C2", label = "SERVAL_corr")
        ...
        ax1.errorbar((ser_rvc[:,0]) % i[3], (ser_rvc[:,1] - np.nanmean(ser_rvc[:,1]) - fit_func(ser_rvc[:,0], T0_offset_s))/ser_rvc[:,2] ,yerr = 0*ser_rvc[:,2]/ser_rvc[:,2]  ,fmt = "x",color = "C2", label = "SERVAL_corr")
        if error_clipping == True:
            ax1.plot((w_dates[ind_bad_wob]) % i[3], (wob_residuals_sign / w_RVs_er)[ind_bad_wob], "o",color = "C0", label = "error_clipped wobble points")
