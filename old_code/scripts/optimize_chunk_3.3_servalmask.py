...
def mask_tellurics(data, mask = telluric_mask):
    """
    Parameters
        ----------
        data : `Data` 
            wobble Data type object from which we will delete the masked sections.
            xs in data are np.log(lambda)
            ys data are np.log(fluxes)
        mask : `numpy array`
            telluric mask array:
            0th: wavelength in nm 
            1st: 1 where active, 0 if not active
    """
    ...
    mask_function = sp.interpolate.interp1d(mask[:,0],mask[:,1])
    mask_array = np.zeros(np.array(data.xs).shape)
    for o, xs_order in enumerate(data.xs):
    ...
    data.xs = np.ma.masked_array(data.xs, mask = mask_array)# log space
    data.ys = np.ma.masked_array(data.ys, mask = mask_array)# log space
    data.ivars = np.ma.masked_array(data.ivars, mask = mask_array)# log space
    data.fluxes = np.ma.masked_array(data.fluxes, mask = mask_array)
    data.flux_ivars = np.ma.masked_array(data.flux_ivars, mask = mask_array)
    #NOTE this may possibly fail, if some of these lists have been differently masked already for low pixels by wobble.Data import
    ...
    for r,o in enumerate(orders):
        ...
        if plots:
            ...
            for e in epochs:
                ...
                xs = np.exp(data.xs[r][e]) #NOTE xs are transferred back from log space (initially transfered in .read_data)
