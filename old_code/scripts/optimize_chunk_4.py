...
if True:
    plots = True
    epochs = [0,50] # to plot # 14_06_1996 no longer passed along
    movies = False
    ...
    data = wobble.Data('../data/'+ starname +'_vis'+'_e2ds.hdf5', orders=orders, epochs=epochs_list ,min_flux=10**-5, min_snr=0 #deprecated, no longer used by this function as of 14_06_2019
                       )
    ...
    ## log and normalize:
    #data.ys = np.log(data.fluxes) 
    #data.continuum_normalize(**kwargs) 
    
    ## HACK - optionally un-log it:
    #if not log_flux:
        #data.ys = np.exp(data.ys)
        #data.ivars = data.flux_ivars
            
    ## mask out high pixels:
    #for r in range(data.R):
        #bad = data.ys[r] > max_norm_flux
        #data.ys[r][bad] = 1.
    ...
    for r,o in enumerate(orders):
        ...
        if plots:
            wobble.optimize_order(model, niter=niter, save_history=True, 
                                  basename=plot_dir+'history', #movies=movies,#deprecated in 14_06_2019
