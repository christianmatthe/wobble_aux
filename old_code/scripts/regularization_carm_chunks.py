...
#epochs_list = np.array(ast.literal_eval(sys.argv[6])) #HACK epochs to load, not necessary in reg I think       
...
if __name__ == "__main__":
    ...
    #starname = 'GJ876'
    orders = np.arange(start_order, end_order) # This seems to break if not startet at 0
    #K_star = 0 # number of variable components for stellar spectrum
    #K_t = 3 # number of variable components for telluric spectrum         
    tellurics_template_fixed = False    
    plot = True
    verbose = True
    ...
    R = len(orders)
    star_filename = '../wobble/regularization/{0}_star_K{1}_orders[{2},{3}).hdf5'.format(starname, K_star, start_order, end_order)
    ...
    tellurics_filename = '../wobble/regularization/{0}_t_K{1}_orders[{2},{3}).hdf5'.format(starname, K_t, start_order, end_order)
    ...
    for r,o in enumerate(orders): # r is an index into the (cleaned) data. o is an index into the 72 orders (and the file tracking them).
        if verbose:
            ...
            with h5py.File(star_filename, 'r') as f:
                for key in list(f.keys()):
                    print("{0}: {1:.0e}".format(key, f[key][r])) # HACK switched o and r (and back)
            print("tellurics:")
            with h5py.File(tellurics_filename, 'r') as f:
                for key in list(f.keys()):
                    print("{0}: {1:.0e}".format(key, f[key][r])) #switched o and r 
        improve_order_regularization(r, r, star_filename, tellurics_filename,
                                         training_data, training_results,
                                         validation_data, validation_results,
        ...
        #HACK switched second input to order reg to r instead of o, hoping this willmake it run with chunks not starting at 0
        if verbose:                                 
            ...
            with h5py.File(tellurics_filename, 'r') as f:
                for key in list(f.keys()):
                    print("{0}: {1:.0e}".format(key, f[key][r])) #switched o and r     
''' Breaks because order inexes are messed up unless they run in a range from 0
