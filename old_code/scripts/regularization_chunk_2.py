    ...
    for component,(tensor,name) in zip(tensor_components, zip(tensors_to_tune, tensor_names)):
        if (name[0:2] == "L1" and L1) or (name[0:2] == "L2" and L2):
            ...
            if component == 'star':
                ...
                assert False
            with h5py.File(filename, 'r+') as f:
                    f[name][o] = np.copy(regularization_dict[tensor]) #This part writes over hdf5 file: why couldn't I do that before?   
...
if __name__ == "__main__":
    ...
    if "reg_file_star" in parameter_dict and "reg_file_t" in parameter_dict: #This duplicates the exception in parameters.py -> redundant
    ...
    #these should be redundant now with manually specified reg_file names
    ...
    # NOTE this setup coulld/should realy be in reg_top
    star_filename = star_reg_file
    tellurics_filename = tellurics_reg_file
    #Write default file if this is the first chunk executed
    #R = len(orders)
    R = 61 #new method should always include full length file even if not all orders are optimized
    if start_chunk == start_top:
        if os.path.isfile(star_filename): #prevent accidentally deleting previous file if erraneously running file
            shutil.copy(star_filename, star_filename +"(backup)") 
        ...
        if os.path.isfile(tellurics_filename):      
            shutil.copy(tellurics_filename, tellurics_filename +"(backup)") 
