...
if __name__ == "__main__":
    ...
    plot_dir = '../results/plots_{0}_order_{1}/'.format(starname,test_order)
    
    if False:
        # quick test on single order
        data = wobble.Data(starname+'_e2ds.hdf5', filepath='../data/', orders=[test_order])
    ...
    for r,o in enumerate(orders[test_order]):
        model = wobble.Model(data, results, r)
        model.add_star('star', variable_bases=K_star, 
                        regularization_par_file=star_reg_file, 
    ...
    results_file = '../results/results_{0}_{1}.hdf5'.format(starname, test_order)
