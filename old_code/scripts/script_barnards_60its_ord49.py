...
if __name__ == "__main__":
    ...
    print("wobble std = {0:.3f} m/s".format(np.std(star_rvs + data.bervs - data.drifts)))
    ...
    if plots:
        ...
        ax.scatter(data.dates, results.star_time_rvs + data.bervs - np.mean(results.star_time_rvs + data.bervs - data.drifts), c='k', label='wobble', alpha=0.7)
        ax.legend()
        ax.set_xticklabels([])
        ax2.scatter(data.dates, results.star_time_rvs  - data.drifts - data.pipeline_rvs, c='k')
        ax2.set_ylabel('JD')
        fig.tight_layout()
        fig.subplots_adjust(hspace=0.05)
